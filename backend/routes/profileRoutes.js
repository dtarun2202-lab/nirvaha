...
// POST /api/profile/pathway-progress
router.post('/pathway-progress', async (req, res) => {
  try {
    const { userId, pathwayId, lessonIndex } = req.body;
    if (!userId || !pathwayId) return res.status(400).json({ error: 'userId and pathwayId required' });

    const user = await User.findOne({ id: userId });
    if (!user) return res.status(404).json({ error: 'User not found' });

    // Initialize pathway progress if it doesn't exist
    if (!user.pathwayProgress) user.pathwayProgress = new Map();
    
    let progress = user.pathwayProgress.get(pathwayId);
    if (!progress) {
      progress = {
        completedLessons: [],
        startedAt: new Date(),
    ...
    // Add lesson index if provided and not already completed
    ...
    user.pathwayProgress.set(pathwayId, progress);

    // Also update stats since this is a learning session
    const today = new Date().toISOString().split('T')[0];
    const activityLog = [...new Set([...(user.stats?.activityLog || []), today])];
    const streak = computeStreak(activityLog);
    const sessionsPlayed = (user.stats?.sessionsPlayed || 0) + 1;
    const totalMinutes = (user.stats?.totalMinutes || 0) + 15; // Assume 15 mins for a pathway moment
    const wellnessScore = wellnessFromStats(sessionsPlayed, streak, totalMinutes);
    ...
    await User.findOneAndUpdate(
      ...
      {
        $set: {
          pathwayProgress: user.pathwayProgress,
          'stats.sessionsPlayed': sessionsPlayed,
          'stats.streak': streak,
          'stats.totalMinutes': totalMinutes,
          'stats.activityLog': activityLog,
    ...
    if (io) {
      io.emit('profile_updated', {
        userId: user.id,
        pathwayProgress: Object.fromEntries(user.pathwayProgress),
        stats: { sessionsPlayed, streak, totalMinutes, wellnessScore }
      });
    }
...
// POST /api/profile/enroll-pathway
router.post('/enroll-pathway', async (req, res) => {
  try {
    ...
    if (user.enrolledPathways.includes(pathwayId)) {
      return res.json({ success: true, message: 'Already enrolled', enrolledPathways: user.enrolledPathways });
    }

    const updated = await User.findOneAndUpdate(
      { id: userId },
      { 
        $addToSet: { enrolledPathways: pathwayId },
        $set: {
    ...
    res.json({ success: true, enrolledPathways: updated.enrolledPathways });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
